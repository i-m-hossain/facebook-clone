<template>
    <div class="bg-white rounded shadow w-2/3 mt-6 overflow-hidden">
        ...
        <div class="px-4 pt-2 flex justify-between text-gray-700 text-sm">
            ...
            <div>
               <button @click="comments =!comments" class="focus:outline-none"><p>{{ post.data.attributes.comments.comment_count }} comments</p></button> 
            </div>
        </div>

        <div class="flex justify-between border-1 border-gray-400 m-4">
            <button @click="$store.dispatch('likePost', {postId:post.data.post_id, postKey: $vnode.key} )" 
        ...
        <div v-if="comments" class="border-t boder-gray-400 p-4 pt-2">
            ...
            <div class="flex my-4 items-center" v-for="comment in post.data.attributes.comments.data" :key="comment.id">
                <div class="w-8">
                    <img src="https://cdn.pixabay.com/photo/2014/07/09/10/04/man-388104_960_720.jpg" 
                        alt="profile image for user" class="w-8 h-8 object-cover rounded-full" >
                </div>
